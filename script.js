...
// 2. DEFINIÇÃO DAS DESPESAS (NOMES DEVEM SER IGUAIS AOS DA PLANILHA)
const CUSTEIO = [
...
// 6. MONTAGEM DA INTERFACE (BUSCA DIRETA NA PLANILHA)
function montarInterface() {
    ...
    const render = (lista, tipo, gridID) => {
        ...
        lista.forEach(nome => {
            // Busca o valor original diretamente pelo nome exato na planilha filtrada
            const r = dadosFiltradosPlanilha.find(x => 
                x[8].normalize("NFD").toUpperCase().trim() === nome.normalize("NFD").toUpperCase().trim()
            );
...
document.getElementById('btn-pdf').onclick = () => {
    ...
    // Função para pegar itens para o PDF (Busca Direta)
    const getItens = (tipo) => {
        const rows = [];
        document.querySelectorAll(`.input-v[data-tipo="${tipo}"]`).forEach(i => {
            const vRef = moneyToFloat(i.value);
            
            // Busca o valor original diretamente na planilha filtrada pelo nome
            const r = dadosFiltradosPlanilha.find(x => 
                x[8].normalize("NFD").toUpperCase().trim() === i.dataset.nome.normalize("NFD").toUpperCase().trim()
            );
